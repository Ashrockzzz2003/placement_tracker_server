...
] = require("../middleware/otpTokenValidator");
...
module.exports = {
        ...
        */
        ...
        try {
            ...
            await db_connection.query(
                queries.userLogin.locks.lockStudentDataAndManagementData,
            );
            ...
            let [student] = await db_connection.query(
                queries.userLogin.queries.checkStudentLoginCredentials,
            ...
            if (student.length > 0) {
                if (student[0].studentAccountStatus === "2") {
                    ...
                    return res.status(401).send({
                        message:
                            "Your Account has been deactivated. Check you mail for further instructions.",
                    });
            ...
            let [manager] = await db_connection.query(
                queries.userLogin.queries.checkManagerLoginCredentials,
                [req.body.userEmail, req.body.userPassword],
            );
            ...
            if (manager.length > 0) {
                ...
                if (manager[0].accountStatus === "0") {
                    ...
                    await db_connection.query(
                        queries.userLogin.locks.lockManagementRegister,
                    );

                    //let [manager_2] = await db_connection.query(`SELECT * from managementRegister WHERE managerEmail = ?`, [req.body.userEmail]);
                    let [manager_2] = await db_connection.query(
                        queries.userLogin.queries.checkIfManagerIsRegistered,
                        [req.body.userEmail],
                    );
                    ...
                    if (manager_2.length === 0) {
                        ...
                        await db_connection.query(
                            queries.userLogin.queries.registerManager,
                            [req.body.userEmail, otp],
                        );
                    } else {
                        //await db_connection.query(`UPDATE managementRegister SET otp = ?, createdAt = ? WHERE managerEmail = ?`, [otp, Date.now(), req.body.userEmail]);\
                        await db_connection.query(
                            queries.userLogin.queries.updateManagerOtp,
                            [otp, Date.now(), req.body.userEmail],
                        );
                    }
            ...
            fs.appendFileSync(
                "logs/errorLogs.txt",
                `${time.toISOString()} - userLogin - ${err}\n`,
            );
        ...
        */
        ...
        async (req, res) => {
            if (
                ...
                req.managerEmail === undefined ||
                req.managerEmail === "" ||
                !validator.isEmail(req.managerEmail) ||
                req.body.otp === null ||
            ...
            try {
                await db_connection.query(
                    `LOCK TABLES managementRegister WRITE, managementData WRITE`,
                );

                let [check_1] = await db_connection.query(
                    `DELETE from managementRegister WHERE managerEmail = ? AND otp = ?`,
                    [req.managerEmail, req.body.otp],
                );
                ...
                await db_connection.query(
                    `UPDATE managementData SET accountStatus = ?, managerPassword = ? WHERE managerEmail = ?`,
                    ["1", req.body.newPassword, req.managerEmail],
                );
                ...
                fs.appendFileSync(
                    "logs/errorLogs.txt",
                    `${time.toISOString()} - loginVerify - ${err}\n`,
                );
                return res
                    .status(500)
    ...
    forgotPassword: async (req, res) => {
        ...
        */
        ...
        try {
            await db_connection.query(
                `LOCK TABLES studentData READ, managementData READ`,
            );
            let [student] = await db_connection.query(
                `SELECT studentName,studentAccountStatus from studentData where studentEmail = ?`,
                [req.body.userEmail],
            );
            let [manager] = await db_connection.query(
                `SELECT managerName,managerRole,accountStatus from managementData where managerEmail = ?`,
                [req.body.userEmail],
            );
            ...
            if (manager.length === 0) {
                ...
                if (student[0].studentAccountStatus === "2") {
                    return res.status(401).send({
                        message:
                            "Your Account has been deactivated by admin due to security reasons. Check you mail for further instructions.",
                    });
                } else if (student[0].studentAccountStatus === "0") {
                    return res.status(401).send({
                        message:
                            "Your Account has not been activated yet. Check you mail for further instructions and login with the password in mail to proceed.",
                    });
                } else if (student[0].studentAccountStatus !== "1") {
                    return res
                        .status(401)
                ...
                let [student_2] = await db_connection.query(
                    `SELECT * from studentRegister WHERE studentEmail = ?`,
                    [req.body.userEmail],
                );

                if (student_2.length === 0) {
                    await db_connection.query(
                        `INSERT INTO studentRegister (studentEmail, otp) VALUES (?, ?)`,
                        [req.body.userEmail, otp],
                    );
                } else {
                    await db_connection.query(
                        `UPDATE studentRegister SET otp = ?, createdAt = ? WHERE studentEmail = ?`,
                        [otp, Date.now(), req.body.userEmail],
                    );
                }
                await db_connection.query(`UNLOCK TABLES`);
                ...
                await db_connection.query(
                    `LOCK TABLES managementRegister WRITE`,
                );
                name = manager[0]["managerName"];
                userRole = manager[0]["managerRole"];
                let [manager_2] = await db_connection.query(
                    `SELECT * from managementRegister WHERE managerEmail = ?`,
                    [req.body.userEmail],
                );

                if (manager_2.length === 0) {
                    await db_connection.query(
                        `INSERT INTO managementRegister (managerEmail, otp) VALUES (?, ?)`,
                        [req.body.userEmail, otp],
                    );
                } else {
                    await db_connection.query(
                        `UPDATE managementRegister SET otp = ?, createdAt = ? WHERE managerEmail = ?`,
                        [otp, Date.now(), req.body.userEmail],
                    );
                }
                await db_connection.query(`UNLOCK TABLES`);
    ...
    resetPasswordVerify: [
        ...
        */
        ...
        async (req, res) => {
            ...
            try {
                await db_connection.query(
                    `LOCK TABLES studentRegister WRITE, managementRegister WRITE, studentData WRITE, managementData WRITE`,
                );

                let check_1 = null;
                if (req.authorization_tier === "2") {
                    [check_1] = await db_connection.query(
                        `DELETE from studentRegister WHERE studentEmail = ? AND otp = ?`,
                        [req.body.userEmail, req.body.otp],
                    );
                } else if (
                    req.authorization_tier === "1" ||
                    req.authorization_tier === "0"
    ...
    resetPassword: [
        ...
        */
        ...
        async (req, res) => {
            ...
            try {
                await db_connection.query(
                    `LOCK TABLES studentData WRITE, managementData WRITE`,
                );

                if (req.authorization_tier === "2") {
                    await db_connection.query(
                        `UPDATE studentData SET studentPassword = ? WHERE studentEmail = ?`,
                        [req.body.userPassword, req.body.userEmail],
                    );
                } else if (
                    req.authorization_tier === "1" ||
                    req.authorization_tier === "0"
                    ...
                    await db_connection.query(
                        `UPDATE managementData SET managerPassword = ? WHERE managerEmail = ?`,
                        [req.body.userPassword, req.body.userEmail],
                    );
                }
    ...
    studentRegister: async (req, res) => {
        ...
        */
        ...
        if (
            req.body.studentEmail === null ||
            req.body.studentEmail === undefined ||
            req.body.studentEmail === "" ||
            !validator.isEmail(req.body.studentEmail) ||
            req.body.studentPassword === null ||
            req.body.studentPassword === undefined ||
        ...
        if (
            ...
            req.body.studentSection === null ||
            req.body.studentSection === undefined ||
            req.body.studentSection === "" ||
            req.body.studentGender === null ||
            req.body.studentGender === undefined ||
            req.body.studentGender === "" ||
            req.body.studentBatch === null ||
            req.body.studentBatch === undefined ||
            req.body.studentBatch === "" ||
            req.body.studentDept === null ||
            req.body.studentDept === undefined ||
            req.body.studentDept === "" ||
            req.body.isHigherStudies === null ||
            req.body.isHigherStudies === undefined ||
            req.body.isHigherStudies === "" ||
            req.body.isPlaced === null ||
            req.body.isPlaced === undefined ||
            req.body.isPlaced === "" ||
            req.body.CGPA === null ||
        ...
        if (
            req.body.studentGender !== "M" &&
            req.body.studentGender !== "F" &&
            req.body.studentGender !== "O"
        ) {
            return res.status(400).send({ message: "Missing details." });
        }

        if (
            req.body.isHigherStudies !== "0" &&
            req.body.isHigherStudies !== "1"
        ) {
            return res.status(400).send({ message: "Missing details." });
        }
        ...
        try {
            ...
            const [student] = await db_connection.query(
                `SELECT * from studentData WHERE studentEmail = ? or studentRollNo = ?`,
                [req.body.studentEmail, req.body.studentRollNo],
            );
            ...
            if (student_2.length === 0) {
                await db_connection.query(
                    `INSERT INTO studentRegister (studentEmail, otp) VALUES (?, ?)`,
                    [req.body.studentEmail, otp],
                );
            } else {
                await db_connection.query(
                    `UPDATE studentRegister SET otp = ?, createdAt = ? WHERE studentEmail = ?`,
                    [otp, Date.now(), req.body.studentEmail],
                );
            }
            ...
            fs.appendFileSync(
                "logs/errorLogs.txt",
                `${time.toISOString()} - studentRegister - ${err}\n`,
            );
            return res.status(500).send({ message: "Internal Server Error." });
        } finally {
            await db_connection.query(`UNLOCK TABLES`);
    ...
    studentVerify: [
        ...
        */
        ...
        async (req, res) => {
            ...
            try {
                await db_connection.query(
                    `LOCK TABLES studentRegister WRITE, studentData WRITE`,
                );
                ...
                //console.log(check);
                const [check_1] = await db_connection.query(
                    `Delete from studentRegister where studentEmail = ? and otp = ?`,
                    [req.body.studentEmail, req.body.otp],
                );
                if (check_1.affectedRows === 0) {
                    await db_connection.query(`UNLOCK TABLES`);
                    return res.status(400).send({ message: "Invalid OTP!" });
                ...
                if (student.length > 0) {
                    ...
                    await db_connection.query(
                        `INSERT INTO studentData (studentRollNo, studentEmail, studentName, studentPassword, studentSection, studentGender, studentBatch, studentDept, isHigherStudies, isPlaced, CGPA, studentAccountStatus) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                        [
                            req.body.studentRollNo,
                            req.body.studentEmail,
                            req.body.studentName,
                ...
                fs.appendFileSync(
                    "logs/errorLogs.txt",
                    `${time.toISOString()} - studentVerify - ${err}\n`,
                );
                return res
                    .status(500)
                    .send({ message: "Internal Server Error." });
